...
function HomeRedirect() {
  const { businesses, setActiveBusiness, logout, setIsAddBusinessOpen } = useAppStore();
  ...
  }, [businesses, navigate, setActiveBusiness]);
...
// Main Layout Wrapper
function MainLayout() {
  const { businesses, activeBusiness, setActiveBusiness } = useAppStore();
  ...
  }, [businessId, businesses, tab, activeBusiness, navigate, setActiveBusiness]);
...
function BaseLayout({ children, currentTab }: { children: React.ReactNode; currentTab: string }) {
  const { businesses, activeBusiness, setActiveBusiness, isDarkMode, toggleDarkMode, logout, user, setIsAddBusinessOpen } = useAppStore();
  const navigate = useNavigate();
...
export default function App() {
  const { isAuthenticated, token, apiBaseUrl, setBusinesses, user } = useAppStore();
